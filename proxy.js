...
async function handleTtsRequest(req, res) {
    // 大模型接口使用 ARK API Key，而不是普通 TTS 的 ACCESS_TOKEN
    ...
    try {
        ...
        console.log(`[BigTTS] 使用 ARK API Key: ${ARK_API_KEY.substring(0, 10)}...`);
        ...
        // 使用 ARK API Key 进行认证（与文本、图片生成相同）
        const headers = {
        ...
        return res.status(500).json({ error: err?.message || 'BigTTS 代理错误', stack: err?.stack });
    }
}
