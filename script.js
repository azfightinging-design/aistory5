...
async function callTextLLM({ storyIdea, pageCount, advancedData }) {
    ...
    const res = await fetch(`${PROXY_BASE}/api/proxy?path=/api/v3/chat/completions`, {
        method: 'POST',
        headers: {
...
async function generateImageByPrompt(prompt) {
    const res = await fetch(`${PROXY_BASE}/api/proxy?path=/api/v3/images/generations`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
function openBookViewer(pages) {
    ...
    async function getOrCreateTtsUrl(text) {
        ...
        try {
            // 通过代理调用火山引擎语音合成大模型
            const res = await fetch(`${PROXY_BASE}/api/proxy?path=/tts`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    text: text,
                    voice: 'ICL_zh_female_nuanxinxuejie_tob', // 火山引擎大模型音色
                    format: 'wav' // 使用 wav 格式，更稳定
                })
            });
            if (!res.ok) {
                let errorDetails = '';
                try {
                    ...
                    errorDetails = await res.text().catch(() => `状态码 ${res.status}`);
                }
                console.error(`[TTS前端] 错误详情:`, errorDetails);
                throw new Error(`TTS 接口错误：${res.status} - ${errorDetails}`);
            }
