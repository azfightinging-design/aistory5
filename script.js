...
async function callTextLLM({ storyIdea, pageCount, advancedData }) {
    ...
    const res = await fetch(`${PROXY_BASE}/api/proxy?path=/chat/completions`, {
        method: 'POST',
        headers: {
...
async function generateImageByPrompt(prompt) {
    const res = await fetch(`${PROXY_BASE}/api/proxy?path=/images/generations`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
function openBookViewer(pages) {
    ...
    async function getOrCreateTtsUrl(text) {
        ...
        try {
            // 通过代理直达 Ark TTS
            const res = await fetch(`${PROXY_BASE}/api/proxy?path=/audio/speech`, {
                method: 'POST',
